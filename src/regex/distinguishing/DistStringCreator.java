...
public class DistStringCreator {
	...
	private static DistinguishingString getDS(Automaton a1, Automaton a2, DSgenPolicy dsGenPolicy,
			Set<String> forbiddenWords, char nicerChar) {
		switch (dsGenPolicy) {
		case RANDOM:
			// if the generated one is null, it generates the other
			boolean pos = rnd.nextBoolean();
			DistinguishingString dsRnd = pos ? getDS(a1, a2, DSgenPolicy.ONLY_POSITIVE, forbiddenWords, nicerChar)
					: getDS(a1, a2, DSgenPolicy.ONLY_NEGATIVE, forbiddenWords, nicerChar);
			if (dsRnd == null) {
				dsRnd = !pos ? getDS(a1, a2, DSgenPolicy.ONLY_POSITIVE, forbiddenWords, nicerChar)
						: getDS(a1, a2, DSgenPolicy.ONLY_NEGATIVE, forbiddenWords, nicerChar);
			}
			return dsRnd;
		case PREF_POSITIVE:
			DistinguishingString dsPrefPos = getDS(a1, a2, DSgenPolicy.ONLY_POSITIVE, forbiddenWords, nicerChar);
			if (dsPrefPos == null) {
				dsPrefPos = getDS(a1, a2, DSgenPolicy.ONLY_NEGATIVE, forbiddenWords, nicerChar);
			}
			return dsPrefPos;
		case PREF_NEGATIVE:
			DistinguishingString dsPrefNeg = getDS(a1, a2, DSgenPolicy.ONLY_NEGATIVE, forbiddenWords, nicerChar);
			if (dsPrefNeg == null) {
				dsPrefNeg = getDS(a1, a2, DSgenPolicy.ONLY_POSITIVE, forbiddenWords, nicerChar);
			}
			return dsPrefNeg;
		case STRICTLY_POSITIVE:
			// we try to generate the negative ds only of the positive one
			// exists
			DistinguishingString strictPos = getDS(a1, a2, DSgenPolicy.ONLY_POSITIVE, forbiddenWords, nicerChar);
			if (strictPos != null && getDS(a1, a2, DSgenPolicy.ONLY_NEGATIVE, nicerChar) == null) {
				return strictPos;
			}
			return null;
		case STRICTLY_NEGATIVE:
			// we try to generate the positive ds only of the negative one
			// exists
			DistinguishingString strictNeg = getDS(a1, a2, DSgenPolicy.ONLY_NEGATIVE, forbiddenWords, nicerChar);
			if (strictNeg != null && getDS(a1, a2, DSgenPolicy.ONLY_POSITIVE, nicerChar) == null) {
				return strictNeg;
			}
			return null;
		case ONLY_POSITIVE:
			String onlyPosDS = getDS(a1, a2, forbiddenWords, nicerChar);
			return onlyPosDS != null ? new DistinguishingString(onlyPosDS, true) : null;
		case ONLY_NEGATIVE:
			String onlyNegDS = getDS(a2, a1, forbiddenWords, nicerChar);
			return onlyNegDS != null ? new DistinguishingString(onlyNegDS, false) : null;
		default:
			break;
		}
	...
	public static DistinguishingString getDS(Automaton a1, Automaton a2, DSgenPolicy dsGenPolicy, char nicerChar) {
		return getDS(a1, a2, dsGenPolicy, Collections.EMPTY_SET, nicerChar);
	}
	...
	public static String getDS(Automaton a1, Automaton a2, Set<String> forbiddenWords, char nicerChar) {
		...
		String word = getExample(a1MinusA2, nicerChar);
		// System.out.println("\t" + word + "\n");
		return word;
	}
		...
		if (SEARCH_MEANINGFUL) {
			...
			do {
				...
				for (char c : nicerChars) {
					//System.out.println("nicer char:" + c);
	...
	public static String getExample(Automaton a, char nicerChar) {
		...
		if (SEARCH_MEANINGFUL) {
			// try to make it nicer
			//
			//System.out.println("nice: " + nicerChar);
			boolean nicest;
			do {
				nicest = true;
				// till there is a ugly char
				if (!result.contains("\0"))
					break;
				// replace the ugly char
				//System.out.println("new:" + nicerChar);
				String mod = result.replaceFirst("\0", Character.toString(nicerChar));
				// it is accepted
				...
				if (a.run(mod)) {
					...
					char[] nicerChars = { ')', '-', '1', 'a', 'B', 'A' }; // to be complete
					for (char c : nicerChars) {
						// replace the ugly char
						mod = result.replaceFirst("\0", Character.toString(c));
						// it is accepted
						if (a.run(mod)) {
							result = mod;
							nicest = false;
